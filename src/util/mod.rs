...
pub fn translate(pos: u64, from_rows: u8, to_rows: u8) -> Result<u64, String> {
    ...
    let mask = position_mask(from_rows)?;
    if pos & !mask != 0 {
        return Err(format!(
            "translate: position {pos} exceeds forest with {from_rows} rows"
        ));
    }
    ...
    let start_from = start_position_at_row(row, from_rows)?;
    let offset = pos.checked_sub(start_from).ok_or_else(|| {
        format!("translate: position {pos} below row start {start_from}")
    })?;
    let start_to = start_position_at_row(row, to_rows)?;
    offset.checked_add(start_to).ok_or_else(|| {
        format!("translate: overflow mapping position {pos} from {from_rows} to {to_rows} rows")
    })
}
...
pub fn is_root_populated(row: u8, num_leaves: u64) -> Result<bool, String> {
    if row >= 64 {
        return Err(format!(
            "is_root_populated: row {row} exceeds u64 bit width"
        ));
    }
    Ok((num_leaves >> row) & 1 == 1)
}
...
mod tests {
    ...
    fn test_translate_invalid_position() {
        ...
        // libFuzzer crash-e483e0da: pos outside from_rows mask caused add overflow.
        assert!(super::translate(0x9494_9494_9494_94c4, 10, 63).is_err());
    }
    ...
    #[test]
    fn test_is_root_populated_rejects_shift_overflow() {
        // libFuzzer position_helpers crash-8cdc5965: row >= 64 panics on >> .
        assert!(super::is_root_populated(64, 0).is_err());
        assert!(super::is_root_populated(255, 1).is_err());
        assert_eq!(super::is_root_populated(0, 1).unwrap(), true);
        assert_eq!(super::is_root_populated(1, 1).unwrap(), false);
        assert_eq!(super::is_root_populated(63, 1u64 << 63).unwrap(), true);
    }
